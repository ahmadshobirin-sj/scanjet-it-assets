...
const AssetCategoryListPage = () => {
    const { props: { asset_categories, table, success, errors }, component } = usePage<SharedData & { asset_categories: ResponseCollection<AssetCategory>; table: TableServerState }>()
    ...
    const handleDelete = (row: AssetCategory) => {
        confirmDialog({
            ...
            description: <>Are you sure you want to delete <b>{row.name}</b> This action cannot be undone.</>,
    ...
    return (
        <AppLayout breadcrumbs={breadcrumbs}>
            <AppContainer className="space-y-6">
                ...
                <AssetCategoryDetailPage assetCategory={assetCategorySelected} ref={assetCategoryDetailPageRef} onClose={() => setAssetCategorySelected(null)} />
                <AssetCategoryUpdatePage assetCategory={assetCategorySelected} ref={assetCategoryUpdatePageRef} onClose={() => setAssetCategorySelected(null)} />
            </AppContainer>
        </AppLayout>
    )
}

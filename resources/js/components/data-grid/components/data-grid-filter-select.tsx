...
import { DataGridSelectFilterField, DataGridSearchResult } from '../data-grid.types';
...
export const DataGridFilterSelect = <TData,>({ field }: DataGridFilterSelectProps<TData>) => {
    ...
    const handleSearch = useCallback(async (searchValue: string): Promise<ExtendedOption[]> => {
        if (field.onSearch) {
                ...
                filteredOptions.forEach(option => {
                    if (option.value !== undefined) {
                        optionsCache.current.set(String(option.value), {
                            ...
                            ...Object.fromEntries(
                                Object.entries(option).filter(([key]) => key !== 'label' && key !== 'value')
                            )
                        });
                    }
                });

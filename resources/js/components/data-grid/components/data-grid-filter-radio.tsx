...
import { DataGridFilterOption, DataGridRadioFilterField } from '../data-grid.types';
        ...
        <div className="space-y-2">
            ...
            <RadioGroup value={filterValue} onValueChange={handleChange}>
                <div className="max-h-48 space-y-2 overflow-y-auto">
                    {field.options
                        ?.filter((option: DataGridFilterOption) => option.value !== undefined)
                        .map((option: DataGridFilterOption, index: number) => (
                            <div key={index} className="flex items-center space-x-2">
                                <RadioGroupItem value={String(option.value)} id={`${field.value as string}-${index}`} />

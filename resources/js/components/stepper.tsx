...
const defineStepper = <const Steps extends Stepperize.Step[]>(...steps: Steps): DefineProps<Steps> => {
                    ...
                    <>
                        ...
                        >
                            <Button
                                ...
                                intent="primary"
                                variant={dataState !== 'inactive' ? 'outline' : 'fill'}
                                size="icon"
...
const Description = ({ children, className, asChild, ...props }: React.ComponentProps<'p'> & { asChild?: boolean }) => {
    ...
    return (
        <Comp date-component="stepper-step-description" className={cn('text-sm text-muted-foreground', className)} {...props}>
            {children}
        </Comp>
    );
};
...
const classForSeparator = cva(
    ['bg-muted', 'data-[state=completed]:bg-primary data-[disabled]:opacity-50', 'transition-all duration-300 ease-in-out'],
    {
        variants: {
            orientation: {
